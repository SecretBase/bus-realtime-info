...
export const getRoutesQueryKey = (params: Params): QueryKey => ['routes', params];
...
const route = async <T>({ companyId, route }: Partial<GetRoutesParams>): Promise<T> => {
...
export const getRoutes = (params: Partial<Pick<GetRoutesParams, 'companyId'>>) =>
	route<APIResponse<Route[], 'RouteList' | 'Route'>>(params);
